...
import { useGetSubsByYear } from "@/app/_features/_admin/_queries/use-get-subs-by-year";
...
export const SubsPerMonthChart = () => {
  ...
  const chartData =
    data?.map((item, index) => ({
      ...
      totalsubs: item.totalsubs, // Número de pedidos
    })) || [];

  return (
    <Card className="w-full p-4">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-lg font-semibold">Assinaturas por mês</h1>
      ...
      {isLoading ? (
        ...
        <ChartContainer config={chartConfig} className="h-[300px] w-full">
          <BarChart data={chartData}>
            ...
            <Bar dataKey="totalsubs" fill="#2563eb" radius={4} />
          </BarChart>
        </ChartContainer>
      )}
    </Card>
